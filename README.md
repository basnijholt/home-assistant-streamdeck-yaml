...
long_press_trigger: threshold  # Trigger while held instead of waiting for release.
pages:
  - name: Home
    buttons:
...
#### Long-press trigger timing

Long presses run when the key is released by default. Set
`long_press_trigger: threshold` to run the long action as soon as
`long_press_duration` is reached:
...
Releasing before the threshold still runs the short action. Once the long
action runs, the eventual release is consumed and cannot activate a button on
a page opened by that long action. Buttons without a configured `long_press`
retain their short action on release, regardless of how long they are held.

Threshold triggering is useful for opening detail pages without waiting for a
release and for hold-to-confirm actions such as shutdown, locking, or completing
a work mode.
...
| `long_press_trigger`   | When to trigger a configured long press action. Use `release` for the existing release-time behavior or `threshold` to trigger as soon as `long_press_duration` is reached.                                                                                                               | `release` | `Literal['release', 'threshold']` |
